...
public class PrintStringUtil {
     ...
     * @return
     */
    public static ArrayList<String> splitEqually(String text, int size, float unitSize, int fontSize, int mappingModel) {
        ...
        int bigSize = size;
        ArrayList<String> ret = new ArrayList<>((length + size - 1) / size);
        for (int start = 0; start < length; start += bigSize) {
        ...
        if (ret.size() > 0) {
            ...
            if (mappingModel == 0) {
                lastValue = PrintStringUtil.padRight(lastValue, size, fontSize, unitSize);
            } else {
                lastValue = PrintStringUtil.padLeft(lastValue, size, fontSize, unitSize);
            }
