...
import com.mwee.android.print.printer.usbserial.util.HexDump;
 ...
 */
class FtdiSerialDriver implements UsbSerialDriver {

    private final UsbDevice mDevice;
    private final UsbSerialPort mPort;

    public FtdiSerialDriver(){
        mDevice = null;
        mPort = null;
    }
    ...
    private class FtdiSerialPort extends CommonUsbSerialPort {
            ...
            try {
                for (int i = 0; i < mDevice.getInterfaceCount(); i++) {
                    if (connection.claimInterface(mDevice.getInterface(i), true)) {
                        PrintLog.d(TAG, "claimInterface " + i + " SUCCESS");
                    } else {
        ...
        public int read(byte[] dest, int timeoutMillis) throws IOException {
            ...
            if (ENABLE_ASYNC_READS) {
                ...
                if (payloadBytesRead > 0) {
                    PrintLog.d(TAG, HexDump.dumpHexString(dest, 0, Math.min(32, dest.length)));
        ...
        public int write(byte[] src, int timeoutMillis) throws IOException {
            ...
            while (offset < src.length) {
                ...
                PrintLog.d(TAG, "Wrote amtWritten=" + amtWritten + " attempted=" + writeLength);
                offset += amtWritten;
            }
            return offset;
        }
