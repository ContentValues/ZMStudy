...
class ProlificSerialDriver implements UsbSerialDriver {
                ...
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread == null) {
                        ...
                        if (readBytes != STATUS_BUFFER_SIZE) {
                            PrintLog.w(TAG, "Could not read initial CTS / DSR / CD / RI status");
                        } else {
                            mStatus = buffer[STATUS_BYTE_IDX] & 0xff;
                        }
        ...
        public void open(UsbDeviceConnection connection) throws IOException {
            ...
            try {
                for (int i = 0; i < mDataInterface.getEndpointCount(); ++i) {
                    UsbEndpoint currentEndpoint = mDataInterface.getEndpoint(i);
                ...
                if (mDevice.getDeviceClass() == 0x02) {
                    ...
                    try {
                        ...
                        if (maxPacketSize0 == 64) {
                            ...
                            PrintLog.w(TAG, "Could not detect PL2303 subtype, "
                                    + "Assuming that it is a HX device");
                            mDeviceType = DEVICE_TYPE_HX;
                        }
                    } catch (NoSuchMethodException e) {
                        PrintLog.w(TAG, "Method UsbDeviceConnection.getRawDescriptors, "
                                + "required for PL2303 subtype detection, not "
                                + "available! Assuming that it is a HX device");
                        mDeviceType = DEVICE_TYPE_HX;
                    } catch (Exception e) {
                        PrintLog.e(TAG, "An unexpected exception occured while trying "
                ...
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread != null) {
                        try {
                            ...
                            PrintLog.e(TAG, "An error occured while waiting for status read thread", e);
                        }
                    }
                }
                resetDevice();
