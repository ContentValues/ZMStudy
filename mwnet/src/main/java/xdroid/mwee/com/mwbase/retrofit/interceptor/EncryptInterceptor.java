package xdroid.mwee.com.mwbase.retrofit.interceptor;
...
public class EncryptInterceptor implements Interceptor {
    ...
    private Request encrypt(Request request) throws IOException {
        ...
        if (requestBody instanceof FormBody) {
            ...
            LogUtil.logNET("加密前请求数据-->" + JSON.toJSONString(formMap));

            encryptStr = MwEncryptaut(JSON.toJSONString(formMap));

        } else {
            LogUtil.logNET("加密前请求数据-->" + JSON.toJSONString(string));
    ...
    private Response decrypt(Response response) {
        ...
        if (response.isSuccessful()) {
            ...
            //todo 默认返回的数据 肯定是加密的 不可能出现 { 开头的数据
            if (string.startsWith("{")) {
                BaseResponse failResponse = JSON.parseObject(string, BaseResponse.class);
                failBackDecrypt(failResponse);
                bodyString = JSON.toJSONString(failResponse);
            } else {
                bodyString = MwDecryptaut(string);
            }

            //todo 再次校验 防止接口抛出非异常业务异常
            JSONObject object = JSON.parseObject(bodyString);
